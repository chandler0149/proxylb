...
impl UdpBackendSession {
    pub async fn send_to(&self, buf: &[u8], target: &TargetAddr) -> io::Result<usize> {
        match self {
            ...
            UdpBackendSession::Shadowsocks {
                socket,
                server_addr,
            } => {
                let ss_addr = crate::outbound::shadowsocks::to_ss_address(target);
                socket
                    .send_to(*server_addr, &ss_addr, buf)
                    .await
                    .map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))
            }
            UdpBackendSession::Socks5 {
                socket,
                backend_relay_addr,
                ..
            } => {
                ...
                let addr_len =
                    crate::outbound::socks5::write_target_addr_to_buf(target, &mut addr_buf)?;
    ...
    pub async fn recv_from(&self, buf: &mut [u8]) -> io::Result<(usize, TargetAddr)> {
        match self {
            ...
            UdpBackendSession::Shadowsocks { socket, .. } => {
                let (len, _server, addr, _extra) = socket
                    .recv_from(buf)
                    .await
                    .map_err(|e| io::Error::new(io::ErrorKind::Other, e.to_string()))?;
            ...
            UdpBackendSession::Socks5 { socket, .. } => {
                ...
                let (target, header_len) =
                    crate::inbound::socks5::parse_target_addr_from_buf(&temp_buf[3..len])?;
