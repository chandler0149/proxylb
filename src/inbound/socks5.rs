...
pub fn parse_target_addr_from_buf(buf: &[u8]) -> std::io::Result<(TargetAddr, usize)> {
    ...
    match buf[0] {
        0x01 => { // IPv4
            if buf.len() < 7 {
                return Err(std::io::Error::new(std::io::ErrorKind::InvalidData, "incomplete IPv4 address"));
            }
            let mut ip = [0u8; 4];
            ip.copy_from_slice(&buf[1..5]);
            let port = u16::from_be_bytes([buf[5], buf[6]]);
            Ok((TargetAddr::Ip(std::net::SocketAddr::new(std::net::IpAddr::V4(std::net::Ipv4Addr::from(ip)), port)), 7))
        }
        0x04 => { // IPv6
            if buf.len() < 19 {
                return Err(std::io::Error::new(std::io::ErrorKind::InvalidData, "incomplete IPv6 address"));
            }
            let mut ip = [0u8; 16];
            ip.copy_from_slice(&buf[1..17]);
            let port = u16::from_be_bytes([buf[17], buf[18]]);
            Ok((TargetAddr::Ip(std::net::SocketAddr::new(std::net::IpAddr::V6(std::net::Ipv6Addr::from(ip)), port)), 19))
        }
        0x03 => { // Domain
            if buf.len() < 2 {
                return Err(std::io::Error::new(std::io::ErrorKind::InvalidData, "incomplete domain length"));
            }
            let len = buf[1] as usize;
            if buf.len() < 2 + len + 2 {
                return Err(std::io::Error::new(std::io::ErrorKind::InvalidData, "incomplete domain or port"));
            }
        ...
        _ => Err(std::io::Error::new(std::io::ErrorKind::InvalidData, "unknown ATYP")),
    }
}

async fn handle_udp_associate<S>(mut client_stream: S, pool: BackendPool, route_idx: Option<usize>) -> anyhow::Result<()>
where
    S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let backend_map: Arc<DashMap<TargetAddr, Arc<crate::udp::UdpBackendSession>>> = Arc::new(DashMap::new());
