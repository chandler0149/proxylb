...
async fn handle_http_connection(
    ...
    let (method, target) = match parse_http_request(&buf[..pos]) {
        Some(res) => res,
        None => {
    ...
    let (mut backend_stream, chosen_traffic) = match crate::inbound::route_and_connect(&pool, &target).await {
        Ok((s, t)) => (s, t),
        Err(e) => {
            tracing::warn!(
                client = %client_addr,
    ...
    crate::inbound::relay_and_track(client_stream, backend_stream, chosen_traffic, Some(stats), &target, "HTTP").await
}
...
async fn read_headers(stream: &mut TcpStream) -> std::io::Result<(Vec<u8>, usize)> {
    ...
    loop {
        ...
        if n == 0 {
            return Err(std::io::Error::new(std::io::ErrorKind::UnexpectedEof, "connection closed before headers received"));
        }
        bytes_read += n;
...
pub fn parse_http_request(headers_raw: &[u8]) -> Option<(String, TargetAddr)> {
    ...
    let target = if path.starts_with("http://") {
        let host_port_part = path.strip_prefix("http://")?
            .split('/')
            .next()?
            .split('?')
            .next()?
        ...
        let host_port_part = path.strip_prefix("https://")?
            .split('/')
            .next()?
            .split('?')
            .next()?
