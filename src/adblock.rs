//! AdBlock domain filtering module.
//!
...
impl AdBlockEngine {
    ...
    pub fn insert(&mut self, domain: &str, is_allow: bool) {
        let domain_lower = domain.to_lowercase();
        let mut current = &mut self.root;
        for part in domain_lower.rsplit('.') {
            if part.is_empty() {
                continue;
            }
    ...
    /// Match a domain against the Trie. Returns `true` if matched by a blocklist rule
    /// and NOT matched by any whitelist (exception) rule.
...
pub struct AdBlockManager {
    pub engine: ArcSwap<AdBlockEngine>,
...
impl AdBlockManager {
    pub fn new(enabled: bool) -> Self {
        Self {
            engine: ArcSwap::from_pointee(AdBlockEngine::new()),
...
pub fn parse_rule_line(line: &str) -> Option<(String, bool)> {
    let line = line.trim();
    ...
    Some((domain.to_lowercase(), is_allow))
}
...
/// Download adblock rules from a URL via a specified backend.
...
/// Compile lists into an `AdBlockEngine`.
pub fn build_engine_from_contents(
    ...
    for content in local_contents.iter().chain(remote_contents.iter()) {
        for line in content.lines() {
            if let Some((domain, is_allow)) = parse_rule_line(line) {
                engine.insert(&domain, is_allow);
                parsed_rules += 1;
            }
        }
    }
    ...
    tracing::info!(
        ...
        allow_rules = engine.allow_rules_count,
        total_parsed = parsed_rules,
        "adblock engine built successfully"
    );
    engine
}

/// Start background manager task to fetch lists periodically.
pub async fn start_adblock_manager(
    manager: Arc<AdBlockManager>,
    pool: BackendPool,
    config: crate::config::AdBlockConfig,
    ...
    for file_path in &config.files {
        match tokio::fs::read_to_string(file_path).await {
            ...
            Err(e) => {
                tracing::error!(file = %file_path, error = %e, "failed to read local adblock file");
            }
        }
    }

    if !local_contents.is_empty() {
        let engine = build_engine_from_contents(&local_contents, &[]);
        manager.engine.store(Arc::new(engine));
    }
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            for url in &config_clone.urls {
                tracing::info!(url = %url, "downloading adblock list");
                match download_url(&pool_clone, config_clone.backend.as_deref(), url).await {
                    Ok(content) => {
                        remote_contents.push(content);
                        tracing::info!(url = %url, "downloaded adblock list successfully");
                    }
                    Err(e) => {
                        tracing::error!(url = %url, error = %e, "failed to download adblock list");
                    }
                }
            }

            let mut current_local_contents = Vec::new();
            for file_path in &config_clone.files {
                if let Ok(content) = tokio::fs::read_to_string(file_path).await {
                    current_local_contents.push(content);
                }
            }

            if !current_local_contents.is_empty() || !remote_contents.is_empty() {
                let engine = build_engine_from_contents(&current_local_contents, &remote_contents);
                manager_clone.engine.store(Arc::new(engine));
            }

            tokio::select! {
                _ = cancel_clone.cancelled() => {
                    tracing::debug!("adblock updater cancelled");
                    return;
                }
                _ = ticker.tick() => {}
            }
...
mod tests {
    ...
    fn test_trie_matching() {
        let mut engine = AdBlockEngine::new();
        engine.insert("example.com", false);
        engine.insert("doubleclick.net", false);
        engine.insert("sub.example.com", true); // exception
    ...
    fn test_adblock_performance_benchmark() {
        ...
        engine.insert("example.com", false);
        engine.insert("sub.example.com", true);
        ...
        println!(
            "\nAdBlock Matcher Benchmark:\n\
             ...
             Avg query time: {:.2} ns\n\
             Queries/second: {:.0}\n",
            engine.block_rules_count + engine.allow_rules_count,
            total_queries,
            duration,
            ns_per_query,
            qps
