...
mod imp {
    ...
    const FLAG_ENABLED: u8 = 0b01;
    const FLAG_BLOCK_PRIVATE: u8 = 0b10;
    ...
    pub struct FilterManager {
        ...
        /// Bitfield: bit 0 = enabled, bit 1 = block_private
        pub flags: AtomicU8,
        ...
        pub blocked_domains_manager: crate::stats::DomainStatsManager,
    }
    ...
    impl FilterManager {
        pub fn new(config: &crate::config::FilterConfig, db_path: Option<&str>) -> Self {
            ...
            if let Some(ref conn) = db {
                ...
                let _ = conn.execute("ALTER TABLE filter_urls ADD COLUMN tag TEXT", []);
                let _ = conn.execute(
                ...
                if let Ok(mut stmt) =
                    conn.prepare("SELECT value FROM filter_state WHERE key = 'enabled'")
                {
                ...
                if let Ok(mut stmt) =
                    conn.prepare("SELECT value FROM filter_state WHERE key = 'block_private'")
                {
                    if let Ok(mut rows) = stmt.query([]) {
                        if let Ok(Some(row)) = rows.next() {
                            let val: String = row.get(0).unwrap();
            ...
            if enabled {
                initial_flags |= FLAG_ENABLED;
            }
            if block_private {
                initial_flags |= FLAG_BLOCK_PRIVATE;
            }

            Self {
                engine: ArcSwap::from_pointee(FilterEngine::new()),
                flags: AtomicU8::new(initial_flags),
                ...
                config_urls: config
                    ...
                    .clone()
                    .into_iter()
                    .map(|u| u.into_filter_url())
                    .collect(),
                backend: config.backend.clone(),
                rebuild_lock: tokio::sync::Mutex::new(()),
                blocked_domains_manager: crate::stats::DomainStatsManager::new(),
            }
        }

        /// Check if target address should be blocked.
        /// Read both flags with a single atomic load.
        #[inline]
        pub fn is_enabled(&self) -> bool {
            self.flags.load(Ordering::Relaxed) & FLAG_ENABLED != 0
        }
        #[inline]
        #[allow(dead_code)]
        pub fn is_block_private(&self) -> bool {
            self.flags.load(Ordering::Relaxed) & FLAG_BLOCK_PRIVATE != 0
        }

        pub fn is_blocked(&self, target: &TargetAddr) -> bool {
            // Single atomic load for both flags.
            let flags = self.flags.load(Ordering::Relaxed);
            if flags & FLAG_ENABLED == 0 {
                return false;
            }

            if flags & FLAG_BLOCK_PRIVATE != 0 && crate::inbound::is_private_target_sync(target) {
                self.blocked_requests.fetch_add(1, Ordering::Relaxed);
                return true;
            }

            // Only domain targets can be blocked — filter lists don't contain raw IPs.
            let host = match target {
                TargetAddr::Domain(host, _) => host,
                TargetAddr::Ip(_) => return false,
            };

            // Load engine guard once to avoid redundant Arc refcount operations.
            let engine = self.engine.load();
            if engine.is_blocked(host) {
                self.blocked_requests.fetch_add(1, Ordering::Relaxed);
                // Zero-alloc parent domain extraction for stats tracking.
                let parent = crate::inbound::extract_parent_domain_str(host);
                self.blocked_domains_manager.increment_block(parent);
                true
            } else {
                false
            }
        ...
        pub fn set_enabled(&self, enabled: bool) {
            if enabled {
                self.flags.fetch_or(FLAG_ENABLED, Ordering::Relaxed);
            } else {
                self.flags.fetch_and(!FLAG_ENABLED, Ordering::Relaxed);
            }
            if let Ok(guard) = self.db.lock() {
                if let Some(conn) = guard.as_ref() {
                    let _ = conn.execute(
        ...
        pub fn set_block_private(&self, block_private: bool) {
            if block_private {
                self.flags.fetch_or(FLAG_BLOCK_PRIVATE, Ordering::Relaxed);
            } else {
                self.flags.fetch_and(!FLAG_BLOCK_PRIVATE, Ordering::Relaxed);
            }
            if let Ok(guard) = self.db.lock() {
                if let Some(conn) = guard.as_ref() {
                    let _ = conn.execute(
        ...
        pub async fn add_rules(&self, rules: &[String]) -> anyhow::Result<()> {
            if let Ok(guard) = self.db.lock() {
                if let Some(conn) = guard.as_ref() {
                    // Begin transaction
        ...
        pub async fn delete_rules(&self, rules: &[String]) -> anyhow::Result<()> {
            if let Ok(guard) = self.db.lock() {
                if let Some(conn) = guard.as_ref() {
                    let _ = conn.execute("BEGIN TRANSACTION", []);
                    for rule in rules {
                        let _ = conn.execute("DELETE FROM filter_rules WHERE domain = ?", [rule]);
                    }
                    let _ = conn.execute("COMMIT", []);
                }
            }
            ...
            if info.is_direct() {
                ...
                match pc {
                    Some(crate::backend::PooledConn {
                        ...
                        Err(_) => {
                            return Ok(
                                crate::outbound::socks5h_connect(&info, target, timeout).await?
                            );
                        }
                    },
                    _ => {
    ...
    pub async fn start_filter_manager(
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let combined_urls: std::collections::HashSet<String> = manager_clone
                    .get_urls()
                    .into_iter()
                    .map(|u| u.url)
                    .collect();
    ...
    mod tests {
        ...
        fn test_rule_parser() {
            ...
            assert_eq!(parse_rule_line("# comment"), None);
            assert_eq!(parse_rule_line("||example.org^"), Some("example.org"));
            assert_eq!(parse_rule_line("@@||example.org^"), None);
            assert_eq!(
            ...
            assert_eq!(parse_rule_line("badsite.com"), Some("badsite.com"));
            assert_eq!(
        ...
        fn test_filter_performance_benchmark() {
            ...
            println!(
             ...
             Avg query time: {:.2} ns\n\
             Queries/second: {:.0}\n",
                engine.block_rules_count, total_queries, duration, ns_per_query, qps
            );
        }
        ...
        fn bench_filter_engine() {
            ...
            println!(
                "Engine built with {} unique domains in {:?}",
                engine.rules.len(),
                start_build.elapsed()
            );
            ...
            println!(
                "Benchmarking {} iterations of lookups ({} total lookups)...",
...
mod stub {
    ...
    impl FilterManager {
        ...
        pub fn is_blocked(&self, _target: &TargetAddr) -> bool {
            false
        }
        pub async fn rebuild_engine(&self) {}
        pub fn set_enabled(&self, _enabled: bool) {}
        pub fn set_block_private(&self, _block_private: bool) {}
        pub async fn add_rules(&self, _rules: &[String]) -> anyhow::Result<()> {
            Ok(())
        }
        pub async fn delete_rules(&self, _rules: &[String]) -> anyhow::Result<()> {
            Ok(())
        }
        pub fn get_rules(&self) -> Vec<String> {
            Vec::new()
        ...
        pub async fn delete_url(&self, _url: &str) -> anyhow::Result<()> {
            Ok(())
        }
        pub fn get_urls(&self) -> Vec<FilterUrl> {
            Vec::new()
    ...
    pub fn parse_rule_line(_line: &str) -> Option<&str> {
        None
    }
